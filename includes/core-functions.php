...
//adding the CSS to the head of the page if there's something
add_Action('wp_head', 'guide_content_head_css');
function guide_content_head_css()
{
    $options = get_option('guide_content_options', guide_content_options_default());
    if (isset($options['custom_css_Class']) && !empty($options['custom_css_Class'])) {
        $customcss = esc_attr($options['custom_css_Class']);
...
function guide_content($atts, $post)
{
    ...
    // Setting Table CSS variable if it exists
    $options = get_option('guide_content_options', guide_content_options_default());
    if (isset($options['custom_table_Class']) && !empty($options['custom_table_Class'])) {
        $customcss = esc_attr($options['custom_table_Class']);
        //	echo $customcss;
    }
    if (isset($options['custom_css_class']) && !empty($options['custom_table_Class'])) {
        $customcss = esc_attr($options['custom_table_Class']);
        //	echo $customcss;
    }
        ...
        if (@$xmlDoc->load($selected_plan) !== false) {
            ...
            foreach ($x->childNodes as $item) {
                if ($item->nodeName == $selected_tab && preg_match('/[a-zA-Z]/', $item->nodeValue) !== false) {
                    ...
                    // Checking for custom CSS to add to Table class
                    $options = get_option('guide_content_options', guide_content_options_default());
                    if (isset($options['custom_table_Class']) && !empty($options['custom_table_Class'])) {
                        $customcss = esc_attr($options['custom_table_Class']);
                        //echo $customcss;
                        $content = str_replace('<table class="sc_sctable', '<table class="sc_sctable ' . $customcss, $content); //adding bootstrap classes for table styling
                        $content = str_replace('<table class="sc_courselist', '<table class="sc_courselist ' . $customcss, $content); //adding bootstrap classes for table styling
            ...
            foreach ($adjust_array as $adjust_pair) {
                ...
                foreach ($adjust_pair_array as $key => $adjustment) { //for each adjust entry in the adjust attribute, make the requested substitutions
                    if (!empty($adjust_pair_array[$key])) {
